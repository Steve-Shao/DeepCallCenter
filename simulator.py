...
from torchsimulator.simulator.ctmdp_base import CTMDPSimulator  
...
class CallCenterSimulator(CTMDPSimulator):
    ...
    def __init__(
        ...
        self._initialize_control_rules()

    def _initialize_control_rules(self):
        """
    ...
    def run_until_time(self, target_time: float) -> None:
        ...
        with torch.no_grad():
            ...
            while torch.any(active_mask):
                ...
                # ========== Overwrite the `run_until_time` method to add the following: ========== #
                original_total_reward_by_class = self.total_reward_by_class.clone()
                # ========== End of Overwrite ========== #
                ...
                # ========== Overwrite the `run_until_time` method to add the following: ========== #
                self.total_reward_by_class = torch.where(
                    active_mask.unsqueeze(0), 
                    self.total_reward_by_class, 
                    original_total_reward_by_class
                )
                # ========== End of Overwrite ========== #
